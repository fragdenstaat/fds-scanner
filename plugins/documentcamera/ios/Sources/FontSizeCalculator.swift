...
// From https://github.com/tbaranes/FittableFontLabel/blob/master/Source/UILabelExtension.swift
...
public class FontSizeCalculator {
  ...
  public func fontSizeThatFits(
    text string: String, maxFontSize: CGFloat = 100, minFontScale: CGFloat = 0.1, rectSize: CGSize
  ) -> CGFloat {
    ...
    let calculatedFontSize = binarySearch(
      font: font, string: string, minSize: minimumFontSize, maxSize: maxFontSize, size: rectSize,
      constraintSize: constraintSize)
    return (calculatedFontSize * 10.0).rounded(.down) / 10.0
  }

  private func binarySearch(
    font: UIFont, string: String, minSize: CGFloat, maxSize: CGFloat, size: CGSize,
    ...
    let rect = string.boundingRect(
      with: constraintSize, options: .usesLineFragmentOrigin, attributes: attributes, context: nil)
    ...
    case .tooBig:
      return binarySearch(
        font: font, string: string, minSize: minSize, maxSize: fontSize, size: size,
        constraintSize: constraintSize)
    case .tooSmall:
      return binarySearch(
        font: font, string: string, minSize: fontSize, maxSize: maxSize, size: size,
        constraintSize: constraintSize)
    }
  }
